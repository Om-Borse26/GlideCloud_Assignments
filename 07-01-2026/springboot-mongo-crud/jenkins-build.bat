...
if "%DOCKERHUB_USER%"=="" (
  echo DOCKERHUB_USER is not set. 1>&2
  popd
  exit /b 1
)
if "%DOCKERHUB_PASS%"=="" (
  echo DOCKERHUB_PASS is not set. 1>&2
  popd
  exit /b 1
)
...
if errorlevel 1 (
  echo Push failed for SHA tag. 1>&2
  popd
  exit /b 1
)
docker push %IMAGE_NAME%:%TAG_BRANCH%
if errorlevel 1 (
  echo Push failed for branch tag. 1>&2
  popd
  exit /b 1
)
